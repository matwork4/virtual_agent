...
_listOfReplies = [
	...
	new Reply(5,1,5,6,"cerises cherry cherries","cerises","チェリー さくらん","memory",-1),
	new Reply(6,1,5,7,"%%favoriteFruit","%%favoriteFruit","%%favoriteFruit","memory",1),
	new Reply(7,1,5,8,"*","*","*","intelligence",-1),
...
function syntaxAnalisys(text, keywords){
	...
	for(var i=0;i<textSplit.length;i++){
		...
		if(keywords.includes(textSplit[i].toLowerCase()) && textSplit[i].length>3){
			return true;
		}
	}
	return false;
